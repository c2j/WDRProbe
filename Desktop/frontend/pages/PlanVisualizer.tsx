...
const KNOWLEDGE_KEYS: KnowledgeEntry[] = [
  // Scans - Specifics must come BEFORE generic 'index scan' or 'seq scan'
  {
    key: "idxOnlyScan",
  ...
  },
  {
    ...
    i18nKey: "vis.kb.agg",
    icon: CalculatorIcon,
    keywords: ["aggregate", "group", "hashaggregate"],
  },
  {
    key: "sort",
    i18nKey: "vis.kb.sort",
...
// Helper icons (simulated components for icons not in lucide import list)
    ...
    <div className="flex flex-col items-center">
      ...
      >
        ...
        <div className="mt-1 flex items-center justify-between">
          <span
            className="font-bold text-xs text-gray-700 truncate max-w-[140px]"
        ...
        <div className="mt-1 grid grid-cols-2 gap-x-2 text-[10px] text-gray-500">
          ...
          <div>
            Rows: <span className="font-mono text-gray-700">{node.rows}</span>
          </div>
        </div>
...
const CostFlowView: React.FC<NodeViewProps> = ({
  ...
  const flatten = (
    n: EnhancedNode,
    depth: number = 0,
  ): Array<{ node: EnhancedNode; depth: number }> => {
        ...
        {flatList.map(({ node: item, depth }) => {
          ...
          return (
            <div
              ...
              className={`grid grid-cols-12 gap-4 p-2 text-xs hover:bg-gray-50 cursor-pointer items-center ${isSelected ? "bg-blue-50" : ""} ${isHighlighted ? "bg-yellow-50" : ""}`}
            >
              <div
...
const PlanVisualizer: React.FC = () => {
  ...
  // Compute Active Knowledge Key based on selection
  const activeKnowledgeKey = useMemo(() => {
    if (!selectedNode) return null;
    const op = selectedNode.operation.toLowerCase();

    // Sort by specificity (e.g., 'index only scan' before 'index scan')
  ...
  // Auto-open knowledge base if user clicks a complex node (optional, here we just scroll if open)
  useEffect(() => {
    if (activeKnowledgeKey && showKnowledgeBase) {
      // Logic handled inside KnowledgePanel via props
    }
  }, [activeKnowledgeKey, showKnowledgeBase]);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-100px)] gap-4">
      ...
      <div className="flex justify-between items-center bg-white p-3 rounded-lg border border-gray-200 shadow-sm shrink-0">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-2 text-[10px] text-gray-500 bg-gray-50 px-2 py-1 rounded border border-gray-200 mr-2">
            ...
            <div className="flex items-center" title=">50% of Total Cost">
              <span className="w-2 h-2 rounded-full bg-[#b91c1c] mr-1"></span>
              &gt;50%
            </div>
            <div className="flex items-center" title=">20% of Total Cost">
              <span className="w-2 h-2 rounded-full bg-orange-500 mr-1"></span>
              &gt;20%
            </div>
            <div className="flex items-center" title=">5% of Total Cost">
              <span className="w-2 h-2 rounded-full bg-yellow-400 mr-1"></span>
              &gt;5%
            </div>
            <div className="flex items-center" title=">1% of Total Cost">
      ...
      <div className="flex flex-1 gap-4 min-h-0">
        {/* SQL Editor */}
        <div
          className={`${getPanelClasses("sql")} flex flex-col bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden transition-all duration-300`}
        >
          <PanelHeader
        ...
        {/* Raw Text Plan Input */}
        <div
          className={`${getPanelClasses("text")} flex flex-col bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden transition-all duration-300`}
        >
          <PanelHeader
        ...
        {/* Visualizer Area */}
        <div
          className={`${getPanelClasses("visual")} flex flex-row bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden relative transition-all duration-300`}
        >
                ...
                {viewMode === "tree" && (
                  <div className="flex items-center bg-gray-200 rounded p-0.5 animate-in fade-in">
                    <button
                      ...
                      className="p-0.5 text-gray-600 hover:bg-white rounded shadow-sm"
                      title="Zoom Out"
                    >
                      <ZoomOut size={12} />
                    </button>
                  ...
                  <div className="py-1">
                    {extractedTables.map((tableName) => (
                      <div
                        ...
                        onClick={() =>
                          setHighlightedTable(
                            highlightedTable === tableName ? null : tableName,
                          )
                        }
                ...
                <div className="flex items-start p-3 bg-red-50 rounded-md border border-red-100 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-red-600 mt-1">
                      Total cost is very high ({plan.cost.toFixed(0)}). The
                      'HashAggregate' and 'Subquery Scan' nodes contribute
                      significantly. Check if partition pruning is effective.
                    </p>
                  </div>
                </div>
              )}
              <p className="text-xs text-gray-500 italic">
                Load a specific SQL to see more targeted indexing advice.
              </p>
            </div>
          </div>
        </div>
