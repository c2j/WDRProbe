...
const KNOWLEDGE_KEYS: KnowledgeEntry[] = [
  {
    key: "diskSpill",
    i18nKey: "vis.kb.diskSpill",
    icon: HardDrive,
    keywords: [],
  },
  {
    key: "cartesian",
    i18nKey: "vis.kb.nestLoop",
    icon: XOctagon,
    keywords: [],
  },
  {
    key: "userFunc",
    i18nKey: "vis.kb.userFunc",
    icon: FunctionSquare,
    keywords: ["func", "fnc"],
  },
  {
    key: "rownum",
    i18nKey: "vis.kb.rownum",
    icon: ListOrdered,
    keywords: ["rownum"],
  },
  {
    key: "idxOnlyScan",
    i18nKey: "vis.kb.idxOnlyScan",
    icon: Zap,
    keywords: ["index only scan"],
  },
  {
    key: "bitmapScan",
    i18nKey: "vis.kb.bitmapScan",
    icon: Layers,
    keywords: ["bitmap heap scan", "bitmap index scan"],
  },
  {
    key: "partIter",
    i18nKey: "vis.kb.partIter",
    icon: Layers,
    keywords: ["partition iterator"],
  },
  {
    key: "idxScan",
    i18nKey: "vis.kb.idxScan",
    icon: Search,
    keywords: ["index scan", "partitioned index scan"],
  },
  {
    key: "seqScan",
    i18nKey: "vis.kb.seqScan",
    icon: AlignLeft,
    keywords: ["seq scan", "tablesample scan"],
  },
  {
    key: "cteScan",
    i18nKey: "vis.kb.cteScan",
    icon: FileCode,
    keywords: ["cte scan"],
  },
  {
    key: "subqueryScan",
    i18nKey: "vis.kb.subqueryScan",
    icon: FileCode,
    keywords: ["subquery scan", "subplan"],
  },
  {
    key: "nestLoop",
    i18nKey: "vis.kb.nestLoop",
  ...
  {
    key: "mergeJoin",
    i18nKey: "vis.kb.mergeJoin",
    icon: GitBranch,
    keywords: ["merge join"],
  },
  {
    key: "append",
    i18nKey: "vis.kb.append",
    icon: LinkIcon,
    keywords: ["append"],
  },
  {
    key: "result",
    i18nKey: "vis.kb.result",
    icon: CheckIcon,
    keywords: ["result"],
  },
  {
    key: "materialize",
    i18nKey: "vis.kb.materialize",
    icon: Database,
    keywords: ["materialize"],
  },
  {
    key: "agg",
    i18nKey: "vis.kb.agg",
    icon: Sigma,
    keywords: ["aggregate", "group", "hashaggregate", "windowagg"],
  },
  {
    key: "sort",
    i18nKey: "vis.kb.sort",
...
const analyzePlan = (
  ...
  const initHits = (id: string) => {
    if (!ruleHits[id]) ruleHits[id] = [];
  };
  ...
  const traverseCheck = (node: EnhancedNode) => {
    ...
    if (op.includes("seq scan") && node.rows > 10000 && node.cost > 100) {
      initHits("Gauss-XN-002");
      ruleHits["Gauss-XN-002"].push(node.uId);
    }

    // 003: SubPlan - Suggestion
    if (op.includes("subplan") || op.includes("subquery scan")) {
      initHits("Gauss-XN-003");
      ruleHits["Gauss-XN-003"].push(node.uId);
    }

    // 004: Cartesian (Nested Loop heuristic) - Risk
    if (op.includes("nested loop")) {
      const hasIndexChild = node.children.some((c) =>
        c.operation.toLowerCase().includes("index scan"),
      );
      if (
        !hasIndexChild &&
        !details.includes("join filter") &&
        !details.includes("index cond")
      ) {
        initHits("Gauss-XN-004");
        ruleHits["Gauss-XN-004"].push(node.uId);
      }
    }
    ...
    if (op.includes("partition iterator")) {
      ...
      if (iterMatch) {
        const count = iterMatch[1].toLowerCase().includes("part")
          ? 9999
          : parseInt(iterMatch[1]);
        if (count > 5) {
          initHits("Gauss-XN-005");
          ruleHits["Gauss-XN-005"].push(node.uId);
        }
    ...
    if (op.includes("bitmap heap scan")) {
      initHits("Gauss-XN-007");
      ruleHits["Gauss-XN-007"].push(node.uId);
    }

    // 008: Disk Spill - Risk
    if (
      details.includes("disk") ||
      details.includes("spill") ||
      details.includes("external merge")
    ) {
      initHits("Gauss-XN-008");
      ruleHits["Gauss-XN-008"].push(node.uId);
    }

    // 009: Index Scan Filter - Suggestion
    if (op.includes("index scan") && details.includes("filter:")) {
      initHits("Gauss-XN-009");
      ruleHits["Gauss-XN-009"].push(node.uId);
    }

    // 011: User Function - Risk
    if (details.includes("func") || details.includes("fnc")) {
      initHits("Gauss-XN-011");
      ruleHits["Gauss-XN-011"].push(node.uId);
    }

    // 012: Update Set Subquery - Suggestion
    if (node.children.length >= 3) {
      const subplans = node.children.filter(
        (c) =>
          c.operation.toLowerCase().includes("subquery") ||
          c.operation.toLowerCase().includes("initplan"),
      );
      if (subplans.length >= 3) {
        initHits("Gauss-XN-012");
        ruleHits["Gauss-XN-012"].push(node.uId);
      }
    }

    // 013: Rownum - Risk
    if (op.includes("rownum") && node.rows > 10000) {
      initHits("Gauss-XN-013");
      ruleHits["Gauss-XN-013"].push(node.uId);
    }
...
const HistorySidebar: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  onLoad: (item: VisHistoryItem) => void;
    ...
    <div className="fixed inset-y-0 right-0 w-80 bg-white shadow-2xl z-50 flex flex-col transform transition-transform duration-300 animate-in slide-in-from-right">
      ...
      <div className="p-2 border-b border-gray-100 flex justify-end">
        <button
          ...
          className="text-xs text-red-500 hover:text-red-700 flex items-center px-2 py-1"
        >
...
const KnowledgePanel: React.FC<{
  ...
  const filteredItems = KNOWLEDGE_KEYS.filter(
    (k) =>
      t(`${k.i18nKey}.title`)
        .toLowerCase()
          ...
          return (
            ...
            >
              <div className="flex items-center mb-2">
                <div
                  className={`p-2 rounded-lg mr-3 ${isActive ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-600"}`}
                >
                  <Icon size={18} />
                </div>
                <h4
                  className={`font-bold text-sm ${isActive ? "text-blue-700" : "text-gray-800"}`}
                >
              ...
              <div className="space-y-2">
                <div className="bg-green-50 p-2 rounded border border-green-100">
                  <div className="flex items-center text-xs font-semibold text-green-700 mb-1">
                    <ThumbsUp size={12} className="mr-1.5" /> {t("vis.kb.pros")}
                  </div>
                  <p className="text-[10px] text-green-800 leading-snug">
                    {t(`${item.i18nKey}.pros`)}
                  </p>
                </div>
                <div className="bg-red-50 p-2 rounded border border-red-100">
                  <div className="flex items-center text-xs font-semibold text-red-700 mb-1">
...
const TreeNode: React.FC<NodeViewProps> = ({
  ...
  const relatesToTable =
    highlightedTable && node.operation.includes(highlightedTable);
  ...
  const hasDiskSpill =
    detailsLower.includes("disk") ||
    detailsLower.includes("spill") ||
    detailsLower.includes("external merge");
  ...
  const showActual =
    planType !== "Explain Only" && node.actualRows !== undefined;
    ...
    <div className="flex flex-col items-center">
      <div
        ...
        className={`
                    ...
                    ${
                      isIssueNode
                        ? "border-red-500 ring-4 ring-yellow-400 bg-yellow-100 scale-105 z-20 animate-pulse"
                        : relatesToTable
                          ? "border-yellow-500 ring-4 ring-yellow-400 bg-yellow-100 scale-105 z-20"
                          : isHoveredCte
                            ? "border-gray-200 ring-2 ring-purple-400 bg-purple-50"
                            : isSelected
      ...
      >
        ...
        <div className="mt-1 flex items-center justify-between">
          <div className="flex items-center">
            {node.nodeId && (
              <span className="text-[9px] text-gray-400 mr-1.5 font-mono">
                #{node.nodeId}
              </span>
            )}
            <span
              className="font-bold text-xs text-gray-700 truncate max-w-[160px]"
          ...
          {hasDiskSpill && (
            <span title="Disk Spill" className="ml-1">
        ...
        <div className="mt-2 grid grid-cols-2 gap-x-2 gap-y-1 text-[10px]">
          {showActual ? (
            <>
              ...
              <div
                className="col-span-1 text-blue-700 font-bold"
                title="Actual Time"
              >
                A-Time:{" "}
            ...
            <>
              ...
              <div
                className="col-span-1 text-gray-600 font-medium"
        ...
        {node.isCteDef && (
          <div className="mt-1 text-[9px] bg-purple-100 text-purple-700 px-1 rounded w-fit">
            CTE: {node.cteName}
          </div>
        )}
    ...
    <div className="bg-white rounded border border-gray-200 overflow-hidden min-w-[800px]">
      ...
      <div className="divide-y divide-gray-100">
        {flatList.map(({ node: item, depth }) => {
          ...
          const isHighlighted =
            highlightedTable && item.operation.includes(highlightedTable);
            ...
            >
              ...
              >
                <div
                  ...
                  onClick={(e) =>
                    item.children.length > 0 && toggleCollapse(item.uId, e)
                  }
                >
                  {item.children.length > 0 ? (
                    collapsedIds.has(item.uId) ? (
                ...
                <div className="flex items-center min-w-0">
                  ...
                  {item.nodeId && (
                    <span className="ml-2 text-[10px] text-gray-400 font-mono shrink-0">
                      #{item.nodeId}
                    </span>
                  )}
                </div>
              ...
              <div className="col-span-2 flex items-center">
                ...
                <span className="ml-2 w-8 text-right text-[10px] text-gray-500">
...
const PlanVisualizer: React.FC = () => {
  ...
  const {
    visSql: sql,
    setVisSql: setSql,
    visRawPlanText: rawPlanText,
    setVisRawPlanText: setRawPlanText,
    visPlan: plan,
    setVisPlan: setPlan,
    visPlanType: planType,
    setVisPlanType: setPlanType,
    visIssues: planIssues,
    setVisIssues: setPlanIssues,
    visViewMode: viewMode,
    setVisViewMode: setViewMode,
  } = usePlanContext();
  ...
  const [visiblePanels, setVisiblePanels] = useState<
    Record<PanelType, boolean>
  >({ sql: false, text: true, visual: true });
  ...
  const scrollToIssues = () => {
    if (issuesPanelRef.current) {
      ...
      setTimeout(
        () =>
          issuesPanelRef.current?.classList.remove("ring-2", "ring-blue-400"),
        1000,
      );
    }
  };
  ...
  const activeKnowledgeKey = useMemo(() => {
    ...
    if (
      ...
      return "diskSpill";
    const match = KNOWLEDGE_KEYS.find((k) =>
      k.keywords.some((kw) => op.includes(kw)),
    );
  ...
  const extractedTables = useMemo(() => {
    ...
    const traverse = (node: EnhancedNode) => {
      ...
      if (match && match[1] && !match[1].startsWith('"*SELECT*'))
        tables.add(match[1]);
  ...
  const parseGaussDBPlan = (
    text: string,
  ): { root: EnhancedNode | null; type: PlanType } => {
    const isTabular =
      text.includes("|") &&
      (text.includes("operation") || text.match(/^\s*\d+\s*\|/m));
    if (isTabular) {
  ...
  const parseTextFormat = (
    ...
    const analyzeRegex =
      /\(actual time=([\d\.]+)\.\.([\d\.]+) rows=(\d+) loops=(\d+)\)/;
    ...
    lines.forEach((line) => {
      ...
      if (operation.startsWith("CTE Scan on")) {
        ...
        operation.startsWith("CTE") &&
        !operation.startsWith("CTE Scan")
      ) {
        isCteDef = true;
  ...
  const parseTabularFormat = (text: string): EnhancedNode | null => {
    const parts = text.split(
      /Predicate Information \(identified by plan id\)/i,
    );
    ...
    if (predicatePart) {
      ...
      pLines.forEach((line) => {
        ...
        if (idMatch) {
          currentId = idMatch[1];
          predicateMap.set(currentId, line.trim());
        } else if (currentId && line.trim()) {
          predicateMap.set(
            currentId,
            predicateMap.get(currentId) + "\n" + line.trim(),
          );
        }
      });
    }
    const lines = planPart
      .split("\n")
      .filter(
        (l) => l.trim() && !l.includes("operation") && !l.startsWith("---"),
      );
    const nodeStack: { node: EnhancedNode; indent: number }[] = [];
    let root: EnhancedNode | null = null;
    let uidCounter = 0;
    ...
    lines.forEach((line) => {
      ...
      const totalCost = costMatch
        ? parseFloat(costMatch[2])
        : cols.length > 5
          ? parseFloat(cols[5].trim())
          : 0;
      ...
      const newNode: EnhancedNode = {
        ...
        selfCost: 0,
        percentage: 0,
        details: predicateMap.get(id) || line.trim(),
        isCteDef,
        isCteScan,
        cteName,
      };
  ...
  const togglePanel = (type: PanelType) => {
    setVisiblePanels((prev) => {
      ...
      if (!next[type] && maximizedPanel === type) setMaximizedPanel(null);
      return next;
    });
  };
  ...
  return (
    <div
      className={
        isFullscreen
          ? "fixed inset-0 z-50 bg-gray-50 flex flex-col p-4 h-screen w-screen overflow-hidden"
          : "flex flex-col h-[calc(100vh-100px)] gap-4"
      }
    >
      {/* Header */}
      <div className="flex justify-between items-center bg-white p-3 rounded-lg border border-gray-200 shadow-sm shrink-0">
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="p-1.5 rounded text-gray-500 hover:text-blue-600 hover:bg-gray-100 mr-2"
            title="History"
          >
            <History size={18} />
          </button>
          <div className="flex items-center space-x-2 text-[10px] text-gray-500 bg-gray-50 px-2 py-1 rounded border border-gray-200 mr-2">
            ...
            <div className="flex items-center" title=">50% Cost">
              <span className="w-2 h-2 rounded-full bg-[#b91c1c] mr-1"></span>
              &gt;50%
            </div>
            <div className="flex items-center" title=">20% Cost">
          ...
          <div className="flex items-center bg-gray-50 rounded border border-gray-200 p-0.5 mr-2">
            {(["sql", "text", "visual"] as PanelType[]).map((type) => (
              <button
                key={type}
                onClick={() => togglePanel(type)}
                className={`px-2 py-1 rounded text-xs font-medium flex items-center transition-colors ${visiblePanels[type] ? "bg-white text-blue-600 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
              >
                {visiblePanels[type] ? (
      ...
      <div className="flex flex-1 gap-4 min-h-0 relative">
        <div
          className={`${getPanelClasses("sql")} flex flex-col bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden`}
        >
          <PanelHeader
        ...
        <div
          className={`${getPanelClasses("text")} flex flex-col bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden`}
        >
          <PanelHeader
            title={t("vis.planText")}
            icon={AlignLeft}
            subtitle="Paste Explain Output"
            type="text"
          />
          <textarea
            ...
            placeholder="Paste GaussDB explain plan here"
          />
        </div>
        <div
          className={`${getPanelClasses("visual")} flex flex-row bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden relative`}
        >
          <div className="flex-1 flex flex-col min-w-0">
              ...
              <div className="flex items-center space-x-4">
                ...
                {viewMode === "tree" && (
                  <div className="flex items-center bg-gray-200 rounded p-0.5">
                    ...
                    <span className="text-[10px] w-8 text-center font-mono text-gray-600">
                      {Math.round(zoom * 100)}%
                    </span>
                    <button
                ...
                <button
                  ...
                  className={`flex items-center px-2 py-0.5 rounded text-[10px] font-medium transition-colors ${showKnowledgeBase ? "bg-blue-100 text-blue-700" : "bg-gray-200 text-gray-600"}`}
                >
                  <BookOpen size={12} className="mr-1.5" /> Knowledge
                </button>
              </div>
              <div className="flex items-center space-x-2">
                {planIssues.length > 0 && (
                ...
                {plan && planType && (
                  <span
                    className={`mr-2 px-2 py-0.5 rounded text-[10px] font-bold border ${
                ...
                {plan && (
                  <span className="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100 font-medium mr-2">
                    {t("vis.totalCost")}: {plan.cost.toLocaleString()}
                  </span>
                )}
                <button
            ...
            <div className="flex-1 relative min-h-0">
              <div
                ref={containerRef}
                className="h-full w-full overflow-auto bg-gray-50 p-6"
              >
                {loading ? (
              ...
              {viewMode === "tree" && extractedTables.length > 0 && plan && (
                <div className="absolute top-4 left-4 z-10 bg-white/95 backdrop-blur shadow-lg border border-gray-200 rounded-lg max-h-[300px] overflow-y-auto w-48 animate-in slide-in-from-left-5">
                  <div className="sticky top-0 bg-white border-b border-gray-100 px-3 py-2 flex items-center text-xs font-semibold text-gray-600">
                    <Table size={12} className="mr-1.5" /> Tables (
                  ...
                  <div className="py-1">
                    {extractedTables.map((tableName) => (
                      <div
                        ...
                        className={`px-3 py-1.5 text-xs cursor-pointer flex items-center justify-between group transition-colors ${highlightedTable === tableName ? "bg-yellow-100 text-yellow-800" : "hover:bg-gray-50 text-gray-600"}`}
                      >
      ...
      {showBottomPanel && !isFullscreen && (
        <div
          ref={issuesPanelRef}
          className="h-48 flex gap-4 min-h-[150px] shrink-0 animate-in slide-in-from-bottom-5 fade-in duration-300"
        >
          <div className="w-full bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col">
            <div className="bg-orange-50 px-4 py-2 border-b border-orange-100 shrink-0 flex justify-between">
              ...
              {planIssues.length > 0 && (
                <span className="text-xs font-bold text-orange-600 bg-orange-100 px-2 rounded-full">
                  {planIssues.length} Issues Found
                </span>
              )}
            </div>
              ...
              {planIssues.length > 0 ? (
                  ...
                  {planIssues.map((issue, idx) => {
                    ...
                    return (
                      ...
                      >
                        <Icon
                          size={16}
                          className={`mr-2 mt-0.5 flex-shrink-0 ${iconColor}`}
                        />
                        <div>
                          <div className="flex items-center space-x-2">
                            ...
                            <span
                              className={`text-[10px] px-1.5 py-0.5 rounded text-white font-medium ${badgeColor}`}
                            >
                              {issue.severity}
                            </span>
                          ...
                          <p className="text-xs text-blue-600 mt-1 font-medium flex items-center">
                            <Info size={12} className="mr-1" /> Suggestion:{" "}
                            {issue.suggestion}
                          </p>
                        </div>
                      </div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-gray-400">
                  ...
                  <span className="text-sm">
                    No critical issues detected by WDRProbe rules.
                  </span>
                </div>
              )}
            </div>
