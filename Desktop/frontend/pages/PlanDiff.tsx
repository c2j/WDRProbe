...
const HistorySidebar: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  onLoad: (item: DiffHistoryItem) => void;
...
const getRiskAssessment = (
  ...
  if (op.includes("seq scan") && node.rows > 10000) {
    risks.push(
      t("diff.risk.largeSeqScan", { rows: (node.rows / 1000).toFixed(1) }),
    );
  }
  ...
  return (
    <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm flex-1">
      ...
      <div className="flex items-end justify-between">
        ...
        <div
          className={`flex items-center px-2 py-1 rounded text-xs font-bold ${bg} ${color}`}
        >
          {icon}
    ...
    <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 w-[600px] bg-white rounded-xl shadow-2xl border border-gray-200 z-50 animate-in slide-in-from-bottom-4 fade-in duration-300 flex flex-col overflow-hidden">
      ...
      <div className="px-4 py-3 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
        <div className="flex items-center space-x-3">
          <span
            className={`px-2 py-0.5 text-xs font-bold uppercase rounded border ${verdictColors[verdictKey]}`}
          >
            {t(`diff.verdict.${verdictKey}`)}
          </span>
          <h3
            className="font-bold text-gray-800 text-sm truncate max-w-[300px]"
            title={opName}
          >
            {opName}
          </h3>
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div className="bg-gray-50 p-2 rounded border border-gray-100">
            <div className="text-xs text-gray-500 mb-1">
              {t("diff.metric.time")} (ms)
            </div>
            <div className="flex items-center justify-between">
              <span className="text-gray-400 line-through text-xs">
                {formatVal(left?.actualTime)}
              </span>
              <ArrowRight size={12} className="text-gray-300" />
              <span className="font-bold text-gray-800">
                {formatVal(right?.actualTime)}
              </span>
            </div>
          </div>
          <div className="bg-gray-50 p-2 rounded border border-gray-100">
            ...
            <div className="flex items-center justify-between">
              <span className="text-gray-400 line-through text-xs">
                {formatVal(left?.totalCost)}
              </span>
              <ArrowRight size={12} className="text-gray-300" />
              <span className="font-bold text-gray-800">
                {formatVal(right?.totalCost)}
              </span>
            </div>
          </div>
          <div className="bg-gray-50 p-2 rounded border border-gray-100">
        ...
        {(leftRisks.length > 0 || rightRisks.length > 0) && (
          <div className="border-t border-gray-100 pt-3">
            <h4 className="text-xs font-bold text-gray-500 uppercase mb-2 flex items-center">
              ...
              {t("diff.analysis.title")}
            </h4>
            <div className="space-y-2 text-xs">
              {/* Comparison of risks */}
              {Array.from(new Set([...leftRisks, ...rightRisks])).map(
                (risk, idx) => {
                  ...
                  if (inLeft && inRight) {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex items-center text-red-600 font-medium bg-red-50 px-2 py-1 rounded"
                      >
                        <AlertCircle size={12} className="mr-1.5" />{" "}
                        {t("diff.analysis.newRisk", { risk })}
                      </div>
                    );
                  } else if (inLeft && !inRight) {
                    return (
                      <div
                        key={idx}
                        className="flex items-center text-green-600 font-medium bg-green-50 px-2 py-1 rounded"
                      >
                        <CheckCircle size={12} className="mr-1.5" />{" "}
                        {t("diff.analysis.resolved", { risk })}
                      </div>
                    );
                  }
                  return null;
...
const DiffFlowNode: React.FC<DiffFlowNodeProps> = ({
  node,
  maxCost,
  depth = 0,
  side,
  ...
  return (
    <div className="flex flex-col font-sans">
      ...
      >
        {/* Operation */}
        <div
          className="col-span-6 flex items-center overflow-hidden"
        ...
        >
          ...
          <span
            className={`truncate font-mono ${isMajorNode ? "text-red-700 font-bold" : "text-gray-700"}`}
        ...
        {/* Metric (Cost or Time) */}
        <div
          className="col-span-2 text-right font-mono text-gray-600 truncate"
          title={metricLabel}
        >
          {primaryMetric}
        </div>
...
const PlanDiff: React.FC = () => {
  ...
  // Context State
  const {
    diffInputMode: inputMode,
    setDiffInputMode: setInputMode,
    diffLeftText: leftText,
    setDiffLeftText: setLeftText,
    diffRightText: rightText,
    setDiffRightText: setRightText,
    diffUnifiedText: unifiedText,
    setDiffUnifiedText: setUnifiedText,
    diffPlanLeft: planLeft,
    setDiffPlanLeft: setPlanLeft,
    diffPlanRight: planRight,
    setDiffPlanRight: setPlanRight,
    diffVerdict: verdict,
    setDiffVerdict: setVerdict,
  } = usePlanContext();
  ...
  const [highlightedUids, setHighlightedUids] = useState<Set<string>>(
    new Set(),
  );
  ...
  const handleNodeClick = (uId: string, side: "left" | "right") => {
    ...
    const rightNode =
      rightUid && planRight ? findNode(planRight, rightUid) : null;
    ...
    if (isTabular) {
      ...
      for (const line of lines) {
        ...
        if (
          ...
          cols[1].toLowerCase().includes("operation")
        )
          continue;
        ...
        if (operation.startsWith("->"))
          operation = operation.substring(2).trim();
  ...
  const detectAndSplit = (text: string): { left: string; right: string } => {
    ...
    if (matches.length >= 2) {
      ...
      return {
        left: text.slice(0, cutPoint).trim(),
        right: text.slice(cutPoint).trim(),
      };
    }
    ...
    lines.forEach((line, idx) => {
      if (textRootRegex.test(line) || tabularRootRegex.test(line)) {
        if (
          ...
          idx > rootIndices[rootIndices.length - 1] + 10
        ) {
    ...
    >
      ...
      {planLeft && planRight ? (
        <div className="flex flex-col flex-1 min-h-0 gap-4">
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex items-center space-x-6 animate-in slide-in-from-top-2 relative z-20">
            ...
            >
              ...
              <span className="font-bold text-sm uppercase">
                {t(`diff.verdict.${(verdict || "Similar").toLowerCase()}`)}
              </span>
            </div>
          ...
          <div
            className="relative flex-1 flex gap-4 min-h-0"
            ref={containerRef}
          >
            {/* SVG Layer for lines */}
            ...
            <div className="flex-1 bg-white rounded-lg border border-gray-200 shadow-sm flex flex-col overflow-hidden relative z-0">
              <div className="p-3 bg-gray-50 border-b border-gray-200 font-semibold text-xs text-gray-600 flex justify-between items-center relative z-20">
                ...
                {planLeft.actualTime && (
                  <span className="text-blue-600 font-mono">
                    {planLeft.actualTime.toFixed(2)}ms
                  </span>
                )}
              </div>
              ...
              <div
                className="flex-1 overflow-auto p-0 relative z-0"
                ref={leftScrollRef}
              >
                <DiffFlowNode
                  node={planLeft}
            ...
            <div className="flex-1 bg-white rounded-lg border border-gray-200 shadow-sm flex flex-col overflow-hidden relative z-0">
              <div className="p-3 bg-gray-50 border-b border-gray-200 font-semibold text-xs text-gray-600 flex justify-between items-center relative z-20">
                <span className="flex items-center">
                  <BarChart2 size={14} className="mr-2 text-green-500" />
                  {t("diff.target")}
                </span>
                {planRight.actualTime && (
                  <span className="text-green-600 font-mono">
                    {planRight.actualTime.toFixed(2)}ms
                  </span>
                )}
              </div>
              <div className="grid grid-cols-12 gap-2 px-2 py-1.5 bg-gray-100 border-b border-gray-200 text-[10px] font-bold text-gray-500 uppercase relative z-20">
            ...
            {inputMode === "unified" ? (
              ...
              <div className="flex flex-1 gap-0">
                <div className="flex-1 flex flex-col border-r border-gray-200">
                  <div className="p-2 bg-gray-50 border-b border-gray-200 font-bold text-gray-600 text-xs uppercase tracking-wide text-center">
                    {t("diff.baseline")}
                  </div>
                  <textarea
