...
fn parse_database_stats(document: &Html) {
    ...
    if let Some(db_table) = document.select(&db_table_selector).next() {
        ...
        for (row_idx, row) in db_table.select(&row_selector).enumerate() {
            ...
            if cells.len() >= 17 {
                db_count += 1;
                let db_name = cells.get(0).unwrap_or(&"".to_string()).clone();
                let backends = cells.get(1).unwrap_or(&"0".to_string()).clone();
                let xact_commit = cells.get(2).unwrap_or(&"0".to_string()).clone();
                let xact_rollback = cells.get(3).unwrap_or(&"0".to_string()).clone();
    ...
    if let Some(load_table) = document.select(&load_table_selector).next() {
        ...
        for (row_idx, row) in load_table.select(&row_selector).enumerate() {
            ...
            if cells.len() >= 3 {
                ...
                let per_second = cells.get(1).unwrap_or(&"0".to_string()).clone();
                let per_transaction = cells.get(2).unwrap_or(&"0".to_string()).clone();
        ...
        if is_sql_table {
            ...
            for (row_idx, row) in table.select(&row_selector).enumerate() {
                ...
                if cells.len() >= 4 {
                    ...
                    let sql_id = cells.get(0).unwrap_or(&"".to_string()).clone();
                    let node_name = cells.get(1).unwrap_or(&"".to_string()).clone();
                    let user_name = cells.get(2).unwrap_or(&"".to_string()).clone();
            ...
            if let Some(element) = document.select(&Selector::parse(selector).unwrap()).next() {
                ...
                for row in element.select(&row_selector) {
                    ...
                    if cells.len() >= 2 {
                        sql_count += 1;
                        if sql_count <= 3 {
                            let sql_text = &cells[1];
                            println!("  SQL {}: {}", sql_count, &sql_text[..sql_text.len().min(60)]);
                        }
                    }
                }
...
fn parse_object_stats(document: &Html) {
    ...
    if let Some(obj_table) = document.select(&obj_table_selector).next() {
        ...
        for (row_idx, row) in obj_table.select(&row_selector).enumerate() {
            ...
            if cells.len() >= 3 {
                obj_count += 1;
                let schema = cells.get(0).unwrap_or(&"".to_string()).clone();
                let name = cells.get(1).unwrap_or(&"".to_string()).clone();
                let obj_type = cells.get(2).unwrap_or(&"".to_string()).clone();
